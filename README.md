...
[![NuGet version (OC.TcPnScanner.CLI)](https://img.shields.io/nuget/v/OC.TcPnScanner.CLI.svg?style=flat-square)](https://www.nuget.org/packages/OC.TcPnScanner.CLI/)
# TcPnScanner
`OC.TcPnScanner.CLI` is a command-line tool written in C#/.NET, that scans for Profinet packets on a network interface.
...
A typical handshake between the `Controller` and `OC.TcPnScanner.CLI` looks like this:
```mermaid
sequenceDiagram
...
```sh
dotnet tool install OC.TcPnScanner.CLI -g
```
outputs
```sh
You can invoke the tool using the following command: OC.TcPnScanner.CLI
Tool 'oc.tcpnscanner.cli' (version '0.1.0') was successfully installed.
```

### Prerequisites
To use `OC.TcPnScanner.CLI`, you need to have `.NET8` or later installed on your machine.
You can download it from the official website: https://dotnet.microsoft.com/download.

## Update
```sh
dotnet tool update OC.TcPnScanner.CLI -g
```

## Uninstall
```sh
dotnet tool uninstall OC.TcPnScanner.CLI -g
```
outputs
```sh
Tool 'oc.tcpnscanner.cli' (version '0.1.0') was successfully uninstalled.
```
...
* Outputs the generated `*.xti`-file
   ```sh
   [16:09:15 INF] 📝 Exported devices to C:\Users\densogiaichned\AppData\Local\Temp\OC.TcPnScanner.CLI\controller (Profinet Device).xti
   ```
...
```sh
OC.TcPnScanner.CLI -f ./samples/sample-capture.pcap | echo
```
outputs
```xml
...
`--log-file`| no | Path to logfile, default `%TEMP%/OC.TcPnScanner.CLI/OC.TcPnScanner.CLI-yyyyMMddhhmm.log`
`--dump`| no | Dump captured packets to file.
`--dump-file`| no | Pcap dump file.
`--aml-file`| no | TIA aml export file.
`--gsd-path`| no | Path to a folder containing GSDML files.
...
```sh
OC.TcPnScanner.CLI -d 00000002-90AB
```

#### Example `-f`/`--pcap-file`
A previously captured `pcap`-file can be loaded, instead of capturing live-data:
```sh
OC.TcPnScanner.CLI -l Information -f samples/sample-capture.pcap -o samples/sample-controller.xti
```
...
```sh
OC.TcPnScanner.CLI -l Verbose -f samples/sample-capture.pcap -o samples/sample-controller.xti
...
ERROR(S):
  ...
  --aml-file              Aml export file from TIA.

  --gsd-path              Path to a folder containing GSDML files.
...
* Windows
  `%TEMP%/OC.TcPnScanner.CLI/OC.TcPnScanner.CLI-yyyyMMddhh.log`
* Linux
  `/tmp/OC.TcPnScanner.CLI/OC.TcPnScanner.CLI-yyyyMMddhh.log`
